...
import { getColumns } from 'components/dashboard/announce/generate/tables/Define';
import { usePath } from 'utils/link';

const GenerateList = (props: any) => {
  const location = useLocation();
  const dispatch = useDispatch();
  const { activedLanguage } = useSelector((state: any) => state.languageReducer); 
  const { list } = useSelector((state: any) => state.generateReducer);
  const { params } = usePath();
  
  const deleteGenerateHandler = useCallback((id: any) => {
  ...
  return useMemo(() => 
    <ResponsiveTable 
      data={list || []} 
      columns={getColumns({t, deleteGenerateHandler, location, activedLanguage})} 
    />
  , [list])
}
