...
export const SearchOrderForm = (props: any) => {
  ...
  const addNewsHandler = () => {
    dispatch(actions.OPEN_DRAWER({ type: ADD_ORDER_DRAWER, size: 'xl', data: {} }));
  }
  ...
  return (
    <form onSubmit={form?.handleSubmit(searchNews)}>
      <Stack spacing={5} direction={{ base: 'column', md: 'row' }}>
        ...
        <FormButton typical="add" permission="order.create" onClick={addNewsHandler} />
        <FormButton type="submit" permission="order.view" typical="search"/>
      </Stack>
    </form>
  )
}
