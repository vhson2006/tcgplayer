...
import { getColumns } from 'components/dashboard/business/product/tables/Define';
...
const ProductList = (props: any) => {
  ...
  const { list } = useSelector((state: any) => state.productReducer);
  const { params } = usePath();

  const deleteProductHandler = useCallback((id: any) => {
  ...
  return useMemo(() => 
    <ResponsiveTable 
      data={list || []} 
      columns={getColumns({t, deleteProductHandler, location, activedLanguage})} 
    />
  , [list])
}
