...
export const SearchCustomerForm = (props: any) => {
  ...
  const addNewsHandler = () => {
    dispatch(actions.OPEN_DRAWER({ type: ADD_CUSTOMER_DRAWER, size: 'xl', data: {} }));
  }
  ...
  return (
    <form onSubmit={form?.handleSubmit(searchNews)}>
      <Stack spacing={5} direction={{ base: 'column', md: 'row' }}>
        ...
        <FormButton typical="add" permission="customer.create" onClick={addNewsHandler} />
        <FormButton type="submit" permission="customer.view" typical="search"/>
      </Stack>
    </form>
  )
}
