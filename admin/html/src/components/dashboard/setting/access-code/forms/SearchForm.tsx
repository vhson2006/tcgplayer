...
export const SearchAccessCodeForm = (props: any) => {
  ...
  const addNewsHandler = () => {
    dispatch(actions.OPEN_DRAWER({ type: ADD_ACCESS_CODE_DRAWER, size: 'xl', data: {} }));
  }
  ...
  return (
    <form onSubmit={form?.handleSubmit(searchNews)}>
      <Stack spacing={5} direction={{ base: 'column', md: 'row' }}>
        ...
        <FormButton typical="add" permission="accesscode.create" onClick={addNewsHandler} />
        <FormButton type="submit" permission="accesscode.view" typical="search"/>
      </Stack>
    </form>
  )
}

export default SearchAccessCodeForm