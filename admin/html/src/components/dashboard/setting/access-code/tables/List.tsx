...
import { getColumns } from 'components/dashboard/setting/access-code/tables/Define';
import { usePath } from 'utils/link';

const AccessCodeList = (props: any) => {
  const location = useLocation();
  const dispatch = useDispatch();
  const { activedLanguage } = useSelector((state: any) => state.languageReducer); 
  const { list } = useSelector((state: any) => state.accessCodeReducer);
  const { params } = usePath();

  const deleteAccessCodeHandler = useCallback((id: any) => {
  ...
  return useMemo(() => 
    <ResponsiveTable 
      data={list || []} 
      columns={getColumns({t, deleteAccessCodeHandler, location, activedLanguage})} 
    />
  , [list])
}
