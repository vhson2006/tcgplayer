...
import { getColumns } from 'components/dashboard/setting/email/tables/Define';
...
const EmailList = (props: any) => {
  ...
  const { list } = useSelector((state: any) => state.emailReducer);
  const { params } = usePath();

  const deleteEmailHandler = useCallback((id: any) => {
  ...
  return useMemo(() => 
    <ResponsiveTable 
      data={list || []} 
      columns={getColumns({t, deleteEmailHandler, location, activedLanguage})} 
    />
  , [list])
}
