...
import { getColumns } from 'components/dashboard/setting/tag/tables/Define';
...
const TagList = (props: any) => {
  ...
  const { list } = useSelector((state: any) => state.tagReducer);
  const { params } = usePath();

  const deleteTagHandler = useCallback((id: any) => {
  ...
  return useMemo(() => 
    <ResponsiveTable 
      data={list || []} 
      columns={getColumns({t, deleteTagHandler, location, lang, activedLanguage})} 
    />
  , [list, lang])
}
