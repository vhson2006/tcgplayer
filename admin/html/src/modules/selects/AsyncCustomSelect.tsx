...
import { ChakraStylesConfig, AsyncSelect, chakraComponents } from "chakra-react-select";
import { RiCloseLine, RiArrowDropDownLine } from "react-icons/ri";
import { t } from "commons/languages/helper";

export const AsyncCustomSelect = (props: any) => {
  const { name, placeholder, isFloating, isRequired, value, errorMessage, fetchData, postExcute, ...others } = props;
  ...
  return (
    ...
    >
      <AsyncSelect 
        ...
        defaultOptions={true}
        value={value}
        loadOptions={async (inputValue: any, callback: any) => {
          let params: any = {};
          if (inputValue !== '') {
            params[name] = inputValue
          }

          let options = await fetchData(params);
          if (postExcute !== undefined) {
            options = postExcute(options);
          }
