import { common, config, defaultLanguage, MAX_REFRESH_TIME } from "commons/config";
import { t } from "commons/languages/helper";
import { StatusCodes, getReasonPhrase } from 'http-status-codes';
import { encryptPermission, loginCheck, sessionCheck } from "utils/account";
...
const renewToken = async (refreshToken: any, activedLanguage = defaultLanguage) => {
  try {
    const response = await fetch(`${config.API_URL}/api/authentication/refresh-tokens`, {
      method: 'PUT',
      headers: {
...
export const uploadFiles = async (files: any, activedLanguage = defaultLanguage): Promise<any> => {
  try {
    if (sessionCheck()) {
      const token = localStorage.getItem('token');
      const formData = new FormData();
      ...
      if (refreshToken) {
        if (loginCheck()) {
          localStorage.removeItem("refresh")
          await renewToken(refreshToken, activedLanguage)
          return uploadFiles(files, activedLanguage)
        } else {
          localStorage.removeItem('token');
          localStorage.removeItem('refresh');
          window.location.assign('/');
...
export const uploadCSV = async (files: any, activedLanguage = defaultLanguage): Promise<any> => {
  try {
    if (sessionCheck()) {
      const token  = localStorage.getItem('token');
      const formData = new FormData();
      ...
      const result = await fetch(`${config.API_URL}/api/product/import`, inputs);
      if (result.status < StatusCodes.INTERNAL_SERVER_ERROR) {
        const response = await result.json();
