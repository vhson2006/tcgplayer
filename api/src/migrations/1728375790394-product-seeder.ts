...
export class ProductSeeder1728375790394 implements MigrationInterface {

  public async up(queryRunner: QueryRunner): Promise<void> {
    const product: Array<any> = await readJSONFile(path.join(__dirname, './data/product.json'));
    ...
    await product.reduce(async (pre: any, cur: any) => {
      const { 
        ...
        name, serial, signal, pack,
        ...productData 
      } = cur;
      const storageData = await queryRunner.manager.createQueryBuilder()
        .select("\"id\"")
        .from(Storage, "storage")
        .where(
          `"storage"."name" = :name AND "storage"."serial" = :serial AND "storage"."signal" = :signal AND "storage"."pack" = :pack`, 
          { name, serial, signal, pack }
        )
        .execute();
      const { identifiers } = await queryRunner.connection.createQueryBuilder()
        ...
        .values({
          ...
          name: name,
          description: name,
          slug: `${name.replace(/[\t\n]/gm, '').replaceAll(' ', '_').replaceAll('/', '_')}_${signal}_${serial}_${pack.replace(/[\t\n]/gm, '').replaceAll(' ', '_').replaceAll('/', '_')}`,
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    const product = await readJSONFile(path.join(__dirname, './data/product.json'));
    await product.reduce(async (pre: any, cur: any) => {
      try {
        const productItem = await queryRunner.manager.createQueryBuilder()
          .select("\"id\", slug")
          .from(Product, "product")
          .where(`"product"."slug" = :slug`, { slug: cur.slug })
          .execute();
        await queryRunner.connection.createQueryBuilder()
          .delete()
